...
func (r *Registrar) formatError(step string, err error) string {
	...
	if strings.Contains(errMsg, "BLOCKED") || strings.Contains(errMsg, "注册请求被拦截") {
		// Step9 send-otp 的 TES BLOCKED 是邮箱域名被拉黑（非 IP 问题），
		// 单独翻译，供 coordinator 走域名熔断（换域名重试）而非代理熔断。
		if step == "SendOTP" {
			return "注册被拦截: 邮箱域名已被拉黑，请更换域名"
		}
...
func (r *Registrar) Run() map[string]interface{} {
	...
	if status == "signup" {
		// 浏览器 WAF 绕过路径已执行 Step7(SIGNUP)+Step7.5(signup init)，
		// tls-client 在 signin.aws 上仍会被 WAF 拦截，故跳过重跑。
		var signupSteps []struct {
			name string
			fn   func() error
		}
		if !r.BrowserSignupDone {
			signupSteps = append(signupSteps,
				struct {
					name string
					fn   func() error
