...
const (
	...
	keyProxy           = "proxy"
	keyGatewayAPIKey   = "gateway_api_key"
)
...
// GetDefaultDataDir 获取默认应用数据目录（标准企业软件惯例：%APPDATA%/kiro-client）
func GetDefaultDataDir() string {
	configDir, err := os.UserConfigDir()
	if err != nil {
		configDir = "."
	}
	return filepath.Join(configDir, "kiro-client")
}
...
func saveConfigMap(m map[string]string) error {
	...
	for _, k := range []string{keyDataDir, keyResultOutputDir, keyProxy, keyGatewayAPIKey} {
...
// getDefaultResultOutputDir 默认输出目录：%APPDATA%/kiro-client/账号
func getDefaultResultOutputDir() string {
	configDir, err := os.UserConfigDir()
	if err != nil {
		configDir = "."
	}
	return filepath.Join(configDir, "kiro-client", "账号")
}

// GetResultOutputDir 获取注册结果输出目录（默认为 %APPDATA%/kiro-client/账号）
...
// ResetResultOutputDir 重置为默认输出目录（%APPDATA%/kiro-client/账号）
...
// GetGatewayAPIKey 返回网关 API Key，未设置时默认 "123"。
func GetGatewayAPIKey() string {
	_gatewayAPIKeyOnce.Do(func() {
		m := loadConfigMap()
		_gatewayAPIKey = strings.TrimSpace(m[keyGatewayAPIKey])
		if _gatewayAPIKey == "" {
			_gatewayAPIKey = "123"
		}
	})
	return _gatewayAPIKey
}

// SetGatewayAPIKey 持久化网关 API Key；空值恢复默认 "123"。
func SetGatewayAPIKey(key string) error {
	key = strings.TrimSpace(key)
	if key == "" {
		key = "123"
	}
	m := loadConfigMap()
	m[keyGatewayAPIKey] = key
	if err := saveConfigMap(m); err != nil {
		return err
	}
	_gatewayAPIKey = key
...
// GetAccountsPath 获取 Outlook 账号文件路径
